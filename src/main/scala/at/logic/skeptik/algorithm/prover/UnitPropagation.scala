...
object UnitPropagation {
  private def isUnit(clause: Sequent): Boolean = clause.width == 1

  // Assumes, that `isUnit` is true
  private def getUnit(clause: Sequent): (Boolean, E) =
    if (clause.ant.size == 1) (true, clause.ant.head) else (false, clause.suc.head)

  // Eliminates one unit clause `negate:unit` in `exp`
  private def eliminate(exp: Set[Sequent], negated: Boolean, unit: E): Set[Sequent] = {
